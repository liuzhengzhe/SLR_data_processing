...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			self.svmtrain_pathname = os.path.join(dirname, '../svmmodule-train')
			self.gnuplot_pathname = '/usr/bin/gnuplot'
		else:
			# example for windows
			self.svmtrain_pathname = os.path.join(dirname, r'..\windows\svmmodule-train.exe')
			# svmtrain_pathname = r'c:\Program Files\libsvm\windows\svmmodule-train.exe'
	...
	def parse_options(self, options):
		...
		if not os.path.exists(self.svmtrain_pathname):
			raise IOError('svmmodule-train executable not found')
...
-svmtrain pathname : set svmmodule executable path and name
...
svm_options : additional options for svmmodule-train""")
		sys.exit(1)
