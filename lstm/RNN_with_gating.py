...
class RNN(object):
    def __init__(self, n_u, n_h, n_y, activation, output_type,
        ...
        self.x = T.concatenate([self.cn.feature_layer.output,self.sampledtraje],axis=1)
        ...
        if self.output_type == 'real':
            ...
            #print 'dimension'
            #print self.y_out.eval({self.x:seq[0]})
    ...
    def nll_multiclass(self, y,n_y):
        ...
        treal=[]
        for i in range(10):
            xreal.append(np.zeros((128*128)))
            treal.append([0,0,0,0,0,0,0,0])

        #print self.y_pred.eval({self.sampledtraje:treal,self.cn.feature_layer.output:xreal})
